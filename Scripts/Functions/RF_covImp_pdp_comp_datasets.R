...
RF.CovImp.pdp.comp.datasets = function(variables, coordinates, dependent_var,
                         var_group, city, mode, nb_test,
                         folds_rf, reps_rf, iters_rf, nb_cores,
                         output_file, output_file_name, save_result, save_RF_object = c("yes")){
  ...
  RF_result$`R-squared` = result$measures.test$rsq
  ...
  if (save_result == "yes"){
    ...
    valuesDepPlot_tot = dependency.plot(RF_mlr_output = result, predictor_dataset = predictor_dataset, variables = variables,
                                        output_file = output_file ,output_file_name = output_file_name, nb_cores = nb_cores)
