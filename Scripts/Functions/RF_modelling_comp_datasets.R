...
rf.modelling.comp.datasets = function(cities, my_cores, mode = NULL, reps_rf = 10, folds_rf = 5, iters_rf = 2, var_group, save_result = 'yes', input_cov, input_malaria, output_file_own, best_var = NULL){
    ...
    for (var_group_test_i in start_GeoSpDt_list:length(GeoSpDt_list)){     # var_group_test_i = 1
      ...
      for (city_nb in 1:length(cities)){
        ...
        if(mode == "ALL"){
          ...
          RF_CovImp_pdp = RF.CovImp.pdp.comp.datasets(variables = variables, coordinates = coords_updated, dependent_var = "PfPR2_10",
  ...
  # -----------------------------------------------------------------------------------------------------------------------
    ...
    for (city_nb in 1:length(cities)){
      ...
      RF_CovImp_pdp = RF.CovImp.pdp.comp.datasets(variables = variables, coordinates = coords_updated, dependent_var = "PfPR2_10",
                                      ...
                                      output_file = output_file, output_file_name = output_file_name, save_result = save_result, save_RF_object = "yes")
